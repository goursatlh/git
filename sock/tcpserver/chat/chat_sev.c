...
int main()
{
	...
	while (iCliIndex < MAX_CLIENT_NUM)
	{
		...
		if (iRet > 0)
		{
			...
			if (pid == 0)
			{//son process
				LOG("LOG info: %s/%d", inet_ntoa(stCliAddr.sin_addr), ntohs(stCliAddr.sin_port));
				if (pthread_create(&tid_s, NULL, send_func, (void *)(&iSonFd[iCliIndex])))
				{
					LOG("create thread for send failed, reason %s", strerror(errno));
					goto EXIT;
				}

				if (pthread_create(&tid_r, NULL, recv_func, (void *)(&iSonFd[iCliIndex])))
				{
